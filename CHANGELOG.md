...
### Added
- **§ 28 Abs. 4 BSIG Aggregation verbundener Unternehmen**: Neue MSP-06-Frage zur IT-Unabhängigkeit des IT-erbringenden Rechtsträgers. Bei IT-Verflechtung mit dem Verbund (geteilte AD/Entra, gemeinsame Admins, zentraler Betrieb) greift eine neue **konsolidierte Schwellenwertprüfung**: Die Rules `REG-M5-KONSOLIDIERT-WICHTIG-VZA`, `REG-M5-KONSOLIDIERT-BESONDERS-VZA` und `REG-M5-KONSOLIDIERT-WICHTIG-FIN` nutzen die Gesamt-VZÄ und den Gesamtumsatz des Verbands (aus den Grunddaten) statt nur die isolierten Kennzahlen des IT-Rechtsträgers. Das entspricht der Verweisung in § 28 Abs. 4 BSIG auf die KMU-Empfehlung 2003/361/EG und der IT-spezifischen Unabhängigkeits-Ausnahme.
- **Neue JurisdictionResult-Flags**: `mspAggregationWarning` (setzt sich bei IT-Verflechtung und erklärt § 28 Abs. 4 BSIG im Ergebnis-Callout) und `mspAggregationApplied` (Betroffenheit erst durch konsolidierte Prüfung ausgelöst).
- **§ 28 Abs. 4 BSIG Callout in ExecutiveSummary**: Prominente Info-Box bei MSP-Pfad mit IT-Verflechtung. Erklärt die Zurechnung verbundener Unternehmen nach KMU-Empfehlung (voll bei verbundenen, anteilig bei Partnerunternehmen) und weist bei `mspAggregationApplied=true` explizit darauf hin, dass die Betroffenheit auf Basis der Verbund-Kennzahlen ausgelöst wurde.
- **Ausführlicher MSP/§ 28 BSIG FAQ-Abschnitt** auf der Hilfe-Seite (`app/hilfe/page.tsx`): sieben neue `<details>`-Einträge mit juristisch präziser Erklärung: MSP-Definition nach § 2 Nr. 26 BSIG (mit BSI-FAQ zur Abgrenzung operativer Betrieb vs. Beratung), Abgrenzung Inhouse/MSP, Schwellenwerte § 28 BSIG inklusive "mindestens 50"-Klarstellung, Zurechnung verbundener Unternehmen (autonom/Partner/verbunden mit 25%/50%-Schwellen), IT-spezifische Unabhängigkeit des § 28 Abs. 4 BSIG inkl. Paradoxon bei Zentral-IT, enge § 28 Abs. 3 BSIG-Ausnahme, Überblick der vier MSP-Prüfschritte.
- **Deutlich schärfere MSP-Fragen-Hints**: MSP-01 erklärt jetzt die Kernfrage »Admin-Zugriff auf IT-Systeme anderer jur. Personen?« und die Abgrenzung zu BSI-FAQ (Beratung reicht nicht); MSP-03 bringt die EU-KMU-Systematik (autonom, Partner, verbunden) für die nachfolgende Aggregationsprüfung; MSP-05 stellt klar, dass die isolierte Schwellenwert-Prüfung nur die halbe Wahrheit ist; MSP-06 enthält den Gesetzeswortlaut § 28 Abs. 4 BSIG inklusive der drei Antwort-Szenarien (unabhängig / verflochten / unklar) und dem Paradoxon bei Zentral-IT für Töchter.
- **Neues Preset** `kv-msp-independent`: DRK-IT-GmbH mit nachweisbar getrennten Systemen (eigener Tenant, eigene Admins, eigene Netze). Demonstriert die IT-Unabhängigkeits-Ausnahme nach § 28 Abs. 4 BSIG.
- **MSP-/IT-Dienstleister-Pfad**: Zweiter Betroffenheits-Pfad neben Rettungsdienst. Eine DRK-Einheit, die zentrale IT für andere juristische Personen (Tochter-gGmbHs, andere KVs, externe Dritte) betreibt, kann nach § 2 Nr. 26 BSIG als Managed Service Provider direkt unter NIS-2 fallen (Sektor »Digitale Infrastruktur«, Anlage 1 BSIG). Neuer Wizard-Abschnitt `msp` mit sechs Fragen: MSP-01 (operative IT-Betriebsleistung für andere jur. Personen?), MSP-02 (Art der Leistung), MSP-03 (Kundenkreis mit Inhouse-Abgrenzung nach Recital 35 NIS-2), MSP-04 (Entgeltlichkeit), MSP-05 (Rechtsträger), MSP-06 (IT-Unabhängigkeit § 28 Abs. 4 BSIG).
- **Neue Regeln im Ruleset**: `REG-M1-NO-MSP`, `REG-M2-INHOUSE` (Inhouse-Ausnahme), `REG-M2-MSP`, `REG-M3-WICHTIG-VZA`, `REG-M3-BESONDERS-VZA`, `REG-M3-WICHTIG-FIN`, `REG-M3-BESONDERS-FIN`. Mirror zu den RD-Regeln, knüpfen an denselben Schwellenwerten (Anlage 1 BSIG).
- **`regulationSector` in JurisdictionResult**: Neues Feld `'none' | 'health' | 'digital_infrastructure' | 'both'`, das den auslösenden Sektor ausdrückt. Alle Ergebnis-Komponenten (ExecutiveSummary, ScopeCards, RegistrationCallout, Vorstand-Briefing) zeigen sektor-spezifische Texte statt hardcodierter Rettungsdienst-Sprache. Neue Key-Facts-Karte »IT-Dienstleister (MSP)« und »Triggernder Sektor« in ExecutiveSummary.
- **Neue Outcome-Variante A-BELOW-THRESHOLDS**: Eigenständiger Outcome-Text für "Sektor getriggert, Schwellenwerte unterschritten" mit Hinweis auf § 28 Abs. 3 BSIG (enge Ausnahme, nicht pauschal entlastend).
- **Neue Presets**: `kv-msp-only` (reiner IT-Dienstleister ohne RD), `kv-msp-inhouse` (Inhouse-IT, kein MSP-Status). Umbenanntes Preset `kv-rd-plus-zentral-it` verdeutlicht Doppelbetroffenheit RD + MSP.
- **Neue Engine-Operatoren** in `lib/rules/evaluate.ts`: `gte`, `lt`, `lt_or_missing` für korrekte Schwellenwert-Prüfung nach § 28 BSIG ("mindestens 50").
- **`visibleIf` mit any/all-Gruppen**: RulesetQuestion-Typ (`VisibilityCondition`) und `AssessmentWizard.isQuestionVisible` unterstützen jetzt verschachtelte Visibility-Bedingungen. Threshold-Fragen (THR-01/02/03) und die BSI-Registrierungsfrage (REG-01) sind dadurch sowohl im RD- als auch im MSP-Pfad sichtbar.

### Fixed
- **Schwellenwert-Bug (Compliance)**: `gt 50` und `gt 250` in `REG-J3-WICHTIG-VZA` / `REG-J3-BESONDERS-VZA` auf `gte 50` / `gte 250` korrigiert. Das Gesetz arbeitet mit "mindestens 50" und "mindestens 250" (§ 28 BSIG). Organisationen mit **exakt** 50 oder 250 VZÄ wurden bisher fälschlich als "Schwellenwerte nicht erreicht" eingestuft — ein echter Compliance-Bug, der jetzt behoben ist. Die alte `REG-J3-NOT`-Regel, die `directlyRegulated` explizit auf `false` setzte, wurde entfernt (Default ist ohnehin `false`, vermied Regel-Reihenfolge-Konflikte zwischen RD- und MSP-Pfad).
- **RD-Only-Hardcoding in Ergebnis-Komponenten**: `ExecutiveSummary`, `ScopeCards`, `RegistrationCallout`, `vorstand-briefing.ts`, `app/ergebnis/page.tsx` und `lib/rules/evaluate.ts` zeigten ausschließlich Rettungsdienst-bezogene Texte. Alle diese Stellen rendern jetzt sektor-abhängig (Gesundheit / Digitale Infrastruktur / beide) und unterscheiden insbesondere die Scope-Logik: bei MSP-only entfällt der "Harte Trennung"-Block, weil der IT-Betrieb selbst Regulierungsgegenstand ist und nicht weg-separiert werden kann.
- **Lieferketten-Hinweis auf Ergebnisseite**: Wird bei Outcome B/C (direkt betroffen) nicht mehr angezeigt (redundant). Erscheint jetzt bei Outcome A (nicht betroffen) und Outcome D (konservative Einschätzung) — in beiden Fällen kann der Lieferketten-Pfad noch relevant sein.
- **Harte Abbruch-Logik in REG-J1**: `OPS-01 = no` setzt nicht mehr `outcome.type = A` direkt, sondern nur noch `jurisdiction.isRdProvider = false`. Das Outcome wird erst nach Auswertung beider Pfade (RD + MSP) in den neuen `REG-OUTCOME-*`-Regeln bestimmt, die beide Sektoren berücksichtigen.
